...
ODA({ is: 'oda-minesweeper', imports: '../../misc/date-timer/date-timer.js',
    template: /*html*/`
        <style>
            :host {
                @apply --vertical;
    ...
    setLevel(level) {
        ...
        switch (level) {
            ...
            case 4:
                ...
                this.rows = Math.floor(h / this.cellSize$def);
                let w = this.offsetParent.offsetWidth - 30;
                this.cols = Math.floor(w / this.cellSize$def);
    ...
    _cellSize() {
        let h = this.offsetParent?.offsetHeight - 110;
        h = (h / this.rows) > this.cellSize$def ? this.cellSize$def : h / this.rows;
        let w = this.offsetParent?.offsetWidth - 30;
        w = (w / this.cols) > this.cellSize$def ? this.cellSize$def : w / this.cols;
        return Math.min(h, w);
    },
    _resize() {
...
ODA({
    ...
    template: /*html*/`
        ...
        <div ~for="rows" class="horizontal flex">
                <oda-minesweeper-mine ~for="cols" :mine="model[cols * $for.index + $$for.index]" ~style="{width: cellSize+'px', height: cellSize+'px'}"></oda-minesweeper-mine>
        </div>
    `
})
    ...
    template: /*html*/`
        <style>
            ...
            .btn {
                ...
                cursor: {{mine?.status === 'opened' ? '$def' : 'pointer'}};
                box-sizing: border-box;
                outline: 1px solid gray;
            }
            .btn:hover {
