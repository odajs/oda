...
ODA({ is: 'oda-flips', imports: '@oda/icon, @oda/button',
    template: `
        ...
        <div  id="board" class="board" @click.stop>
            <div ~for="[...Array(+row)?.keys()] || []" class="row">
                <div ~for="[...Array(+column)?.keys()] || []" class="cell" ~class="{_selected: (solved.includes(column * $for.item + $$for.item)) || (column * $for.item + $$for.item) === card1?.id || (column * $for.item + $$for.item) === card2?.id, solved: solved.includes(column * $for.item + $$for.item)}"
                        @click.stop="ontap($event, (column * $for.item + $$for.item), cards?.[(column * $for.item + $$for.item)])">
                    <div class="cell-inner">
                        <div class="cell-front" ~style="{color: 'hsla(' + (cards?.[column * $for.item + $$for.item]?.c || 0) + ', 60%, 50%, 1)'}">
                            <div ~is="mode === 'images' || mode === 'colors' ||(column * $for.item + $$for.item) === odd? 'img' : 'div'" :src="cards?.[(column * $for.item + $$for.item)]?.v || './icon.png'" style="max-width: 100%; max-height: 100%;">{{cards?.[(column * $for.item + $$for.item)]?.v}}</div>
                        </div>
                        <div class="cell-back" style="display: flex; align-items: center; justify-content: center">
                            <img ~if="(column * $for.item + $$for.item) === odd" src="./icon.png" style="max-width: 100%;max-height: 100%;">
                        </div>
                    </div>
                </div>
            </div>
    ...
    `,
    $pdp: {
        row: { $def: 3, $save: true, $group: 'settings' },
        column: { $def: 3, $save: true, $group: 'settings' },
        mode: { $def: 'images', $save: true, $group: 'settings' },
        timeToClose: { $def: 750, $group: 'settings' },
        babyMode: { $def: false, $save: true, $group: 'settings' },
    ...
    init() {
        ...
        const images = [];
        let url = path.replace('flips.js', 'cards/cards-');
        for (let i = 1; i <= 140; i++) { images.push(url + (i < 10 ? '00' + i : i < 100 ? '0' + i : i) + '.jpg') }
        const colors = [];
        url = path.replace('flips.js', 'colors/colors-');
