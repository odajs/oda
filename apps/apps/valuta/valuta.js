...
ODA({
            ...
            <div class="horizontal between" style="overflow: auto">
                <div class="vertical">
                    <div class="container" style="overflow:auto;flex:1;font-size:small;">
                        ...
                        <div ~for="arrValute()" ~class="{odd:!($for.index%2),even:$for.index%2,focused:focusedValute?.CharCode===$for.item.CharCode}" class="row" style="display:flex; justify-content: space-between;text-align:right"
                                @tap="_tapValute($for.item)">
                            <div class="cell" style="flex:1;text-align:left">{{$for.item.CharCode}}</div>
                            <div class="cell" style="flex:5;text-align:left">{{$for.item.Name}}</div>
                            <div class="cell" style="flex:2">{{$for.item.Nominal}}</div>
                            <div class="cell" style="flex:2">{{$for.item.Value}}</div>
                            <div class="cell" style="flex:1" ~style="{color:$for.item._Def>=0?'blue':'red'}">{{$for.item.Def}}</div>
                        </div>
                    </div>
                </div>
                <div class="container" style="overflow:auto;flex:1;font-size:small;max-width:240px;min-width:240px;">
                    ...
                    <div ~for="arrPeriod() || []" ~class="{odd:!($for.index%2),even:$for.index%2,focused:focusedDate?.x===$for.item.x}" class="row" style="display:flex; justify-content: space-between;text-align:right" @tap="_tapDate($for.item)">
                        <div class="cell" style="flex:2;text-align:center">{{$for.item.x}}</div>
                        <div class="cell" style="flex:1">{{$for.item.n}}</div>
                        <div class="cell" style="flex:1">{{$for.item.y}}</div>
                    </div>
                </div>
            </div>
        </div>
        <oda-chart ~if="_isReady" type="line" :data="data" :options="options"></oda-chart>
    `,
    $public: {
        startValute: 'USD',
        data: Object,
    ...
    async ready() {
        ...
        // let json = await fetch(`https://thingproxy.freeboard.io/fetch/https://www.cbr-xml-daily.ru/daily_json.js`);
        ...
        const res = await fetch(`https://thingproxy.freeboard.io/fetch/https://www.cbr.ru/scripts/XML_dynamic.asp?date_req1=${startDate}&date_req2=${endDate}&VAL_NM_RQ=${this.focusedValute?.ID}`);
