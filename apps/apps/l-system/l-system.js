...
ODA({ is: 'oda-l-system', extends: 'oda-app-layout',
    template: `
        ...
        <div slot="left-panel" class="vertical flex border" style="overflow: auto;">
            <oda-button ~for="_data" @tap="name=$for.item" ~class="{header:item===name}">{{$for.item}}</oda-button>
        </div>
    ...
    $public: {
        name: {
            $type: String, $def: 'tree', $group: 'actions', $save: true,
            set(n) {
                if (n) {
                    this._refreshPage();
                    this.getCommands(n, true);
                }
            }
        },
        animation: {
            $type: Boolean, $group: 'actions',
        ...
        x: { $type: Number, $def: 0, $group: 'offset' },
        y: { $type: Number, $def: 0, $group: 'offset' },
        orientation: { $type: Number, $def: 0, $group: 'offset' },
        levels: {
            $type: Number, $def: 0, $group: 'params',
            set(v) { this.getCommands() }
        },
        sizeValue: { $type: Number, $def: 0, $group: 'params' },
        angleValue: { $type: Number, $def: 0, $group: 'params' },
        rules: {
            $type: Object, $group: 'params',
            set(v) { this.getCommands() }
        },
        symbols: {
            $type: Object, $group: 'params',
        ...
        rotate: { $type: Number, $def: 0, $group: 'params' },
        lineWidth: { $type: Number, $def: 0.218, $group: 'variables' },
        lineColor: {
            $type: String, $def: 'black', $group: 'variables',
            $list: ['red', 'blue', 'green', 'orange', 'lightblue', 'lightgreen', 'lightyellow', 'yellow', 'darkgray', 'gray', 'darkgray', 'lightgray', 'white', 'black'],
        },
        colorStep: { $type: Number, $def: 0, $group: 'variables' },
        depth: { $type: Number, $def: 0, $group: 'variables' },
        sizeGrowth: { $type: Number, $def: 0, $group: 'variables' },
        angleGrowth: { $type: Number, $def: 0, $group: 'variables' },
        speed: { $type: Number, $def: 1, $group: 'variables' },
        _lenght: 0,
        categories: {
            $type: Array,
            $def: ['actions', 'offset', 'params', 'variables']
        },
        showButtons: Boolean,
        canvas: {
            get() {
    ...
    toUrl() {
        ...
        url = path.replace('l-system.js', url);
    ...
    _refreshPage(sure = false) {
        ...
        const url = path.replace('l-system.js', 'index.html');
        if (sure)
