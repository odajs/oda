...
ODA({imports: '@oda/code-editor, @oda/button',
    ...
    `,
    $public: {
        _widthL: { $type: Number, $def: 600, $save: true },
        src: '',
        lzs: '',
    ...
    _open() {
        let url = path.replace('html-live.js', 'index.html#?') + LZString.compressToEncodedURIComponent(this.editor.getValue());
        window.open(url, '_blank').focus();
    },
    _reload() {
        window.location.href = path.replace('html-live.js', 'index.html#?') + LZString.compressToEncodedURIComponent(this.editor.getValue());
