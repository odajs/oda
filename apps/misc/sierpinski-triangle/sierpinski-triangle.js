...
ODA({
    ...
    template: /*html*/`
        ...
        <div class="outerDiv" id="outerDiv">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1000" style="background: #ccc;">
                ...
                <circle ~for="dots.length" :cx="dots[$for.index].x" :cy="dots[$for.index].y" :r="radiusDot" ~style="'fill:'+dots[$for.index].color"></circle>
                <circle ~for="triangleVertices.length" :id="$for.index" :cx="triangleVertices[$for.index].x" :cy="triangleVertices[$for.index].y" :r="radiusVertex"
                        ~style="'fill:'+triangleVertices[$for.index].color" @track="move" class="vertex"></circle>
            </svg>
        </div>
        `,
    $public: {
    ...
    attached() {
        this.async(() => {
            ...
            resizeObserver.observe(this.$('#outerDiv'));
        }, 100)
    },
    onSizeChanged( entries ) {
        this.fontSize = 16 * (this.$('svg')?.viewBox.baseVal.width / this.$('svg')?.getBoundingClientRect().width);
        },
    start() {
    ...
    move(e,d) {
        ...
        this.debounce( "key", this.recalculateDots.bind(this), 33);
    },
})
