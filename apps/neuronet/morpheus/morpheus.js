    ...
    getTokenItem(token){
        ...
        if (!item){
            ...
            if (!item){
                ...
                item.net = [];
                item.net.push(this.array(this.dim).map(i=>{
                    return this.array(this.dim).map(i=>this.initWeight());
                }));
                // item.net.push(this.array(this.dim / 8).map(i=>{
                //     return this.array(this.dim).map(i=>this.initWeight());
                // }));
                // item.net.push(this.array(this.dim).map(i=>{
                //     return this.array(this.dim).map(i=>this.initWeight());
...
function multiplyMatrix(m1, m2, width, out, height){
    height ??= m1.length / width;
