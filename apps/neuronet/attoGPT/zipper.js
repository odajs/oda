// Сам класс new Zipper(<fun>,<argsDem>) создает класс наследуемый от функции и который можно использовать как функцию
...
class Zipper extends Function{
    ...
    __call__() {
        ...
        let err0 = (n1,n2) => {console.error(`Zipper: Ошибка, количество аргументов функции ${n1}, но подали ${n2}.`)}
        if (this.argsDem.length != args.length) {err0(this.argsDem.length,args.length); return} // проверили кол-во аргументов
        // ! тут должна быть ко расширение при не строгой размерности, но позже (и без этого почти работает)
        ...
        // console.log('argShapes=',argShapes)
        let err2 = (nom, l ,l1,l2) => {console.error(`Zipper: Ошибка аргумента ${nom} на уровне ${l}. Размерность ${l1}, но есть ${l2}.`)}
        ...
        // console.log('maskShapes=',maskShapes)
...
function testTen(shape,filling) {
    ...
    else return (new Array(shape[0])).fill(0).map(() => testTen(shape.slice(1),filling) )
}
// Набор тестовых аргументов
let x = testTen([2,3,1],()=>2)
