...
```warning_md
В Web-хранилище могут сохраняться только данные простых типов. Структурные типы (массивы и объекты) не сохраняются.
```
...
ODA({
    is: 'test-component',
    template: `
        <input ::value="value[0]" placeholder="Введите текст">
        <button @tap="clear">Очистить</button>
    `,
    value: {
        $def: [''],
...
В данном примере введенное в строку ввода значение передается в элемент с индексом **0** массива **value**, который объявлен с модификатором **$save**. Введите в строку ввода что-нибудь и перезагрузите страницу, чтобы убедиться, что данные из массива **value** не восстанавливаются, т.к. не сохраняются в хранилище.
