...
1. в виде объекта, в котором имена свойств совпадают с именами классов, а значениями свойств являются **true** (разрешает применение класса к элементу) или **false** (запрещает применение класса к элементу).
    ...
    ```warning_md
    Имя CSS-класса является чувствительным к регистру, поэтому директива **~class** не поддерживает преобразование имен свойств связанного с ней объекта из верблюжьей нотации (Camel case) в шашлычную (Kebab case). В результате, с помощью верблюжьей нотации нельзя передать в директиву имена классов, содержащих дефис (**-**).
    ```
