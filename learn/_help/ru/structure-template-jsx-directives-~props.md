...
```info_md
В директиве **~props** можно не указывать связанный с ней объект с атрибутами. В этом случае компонент должен содержать объект с именем **props**, который автоматически будет привязан к директиве.
```
