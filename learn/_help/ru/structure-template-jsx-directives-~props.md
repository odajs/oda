...
Если HTML-атрибут применяется без указания значения, то для его создания в привязанном объекте необходимо объявить одноименное свойство со значением **true** или значением, которое приводится к значению **true**.
...
В данном примере в объект **attr** добавлено свойство **disabled**. Если открыть *инструменты разработчика* в браузере, то можно убедиться, что это свойство превратилось в атрибут **disabled** без значения.

```html
<my-component>
    #shadow-root (closed)
        <input type="range" min="0" max="100" step="50" disabled>
</my-component>
```

Если свойство, соответствующее атрибуту без значения, объявить со значением **false**, то атрибут создан не будет.
...
Данный пример аналогичен предыдущему, только свойство **disabled** в объект **attr** имеет значение **false**. В результате атрибут **disabled** не был создан. В этом можно убедиться, открыв *инструменты разработчика*.
...
Значения атрибутов, задаваемые директивой биндинга, имеют приоритет над значениями атрибутов, устанавливаемых директивой **~props**.
...
В данном примере в элементе **input** использована директивы биндинга для присвоения значения атрибуту **value**. Как видно по результату выполнения примера, значение устанавливаемое директивой было проигнорировано.
