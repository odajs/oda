...
1. Обработка неопределенных элементов массивов.
    ...
    ```info_md_hideicon
    Для исключения создания HTML-элементов для неопределенных значений из источника данных можно использовать директиву **~if="$for.item"**, которая будет отфильтровывать неопределенное элементы.
    ```
...
1. Обработка отсутствующих элементов массивов.
    ...
    ```info_md_hideicon
    Директива **~for** считает отсутствующий индекс концом массива и прекращает его обработку, даже если массив имеет элементы со старшими индексами.
    ```
    ...
    В этом примере, при создании элементов массива **array** в хуке **ready**, пропущен элемент с индексом **1**. Директива **~for** создает HTML-элемент **div** для элемента массива с индексом **0** и, не обнаружив очередного ожидаемого элемента с индексом **1**, прекращает работу. В результате HTML-элемент **div** для элемента массива с индексом **2** создан не будет.
