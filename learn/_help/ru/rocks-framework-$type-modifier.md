...
```warning_md
Обратите внимание, что при присвоении свойству со статическим типом **Number** значения **NaN**, фреймворк автоматически преобразует его в число **0**. Поэтому будьте внимательны при присвоении значений строкового типа такому свойству. Если строка не может быть преобразована в число, то свойство получит значение **0**, а не **NaN**, как в нативном JavaScript.
```
...
<script type="module">
    ...
    myObject.nativeCounter = +"Абракадабра";
    div1.innerText = "Значение свойства из ROCKS: " + myObject.rocksCounter + " — Тип:" + typeof myObject.rocksCounter;
    div2.innerText = "Значение нативного свойства: " + myObject.nativeCounter + " — Тип:" + typeof myObject.nativeCounter;
</script>
```

В данном примере в функции **ROCKS** объявлено свойство **rocksCounter**. С помощью модификатора **$type** ему задан статический тип **Number**. В коде примера ему присваивается значение строкового типа "Абракадабра". Для выполнения операции присваивания интерпретатор языка JavaScript преобразует эту строку в число и получает значение **NaN**, которое фреймворк преобразует в число **0**. Для сравнения в нативной части класса объявлено свойство **nativeCounter**, которому тоже присваивается строка "Абракадабра". Как видно по результатам выполнения примера, в нативном свойстве преобразования значения **NaN** в **0** не происходит.

```warning_md
Обратите внимание, что при присвоении свойству со статическим типом **Boolean** значения строкового типа, используется алгоритм преобразования типов, который отличается от используемого в конструкторе **Boolean()**. Этот конструктор преобразует любую непустую строку в логическое значение **true**, а пустую – в **false**. Во фреймворке в значение **true** преобразуется только строка **"true"**, причем регистр символов роли не играет. Например, строка **"tRUe"** тоже примет значение истина.
```
