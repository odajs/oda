...
В примере геттер **getterTime** не содержит в своем коде обращение к счетчику **change**, но он вызывает метод **hiddenChange**, в котором такое обращение присутствует, поэтому при любом изменении текста в строке ввода обновляется текущее время на странице.
