...
В примере геттер **getterTime** не содержит в своем коде обращение к счетчику **change**, но он вызывает метод **hiddenChange**, в котором такое обращение присутствует. Поэтому при любом изменении текста в строке ввода текущее время на странице будет обновляться.
