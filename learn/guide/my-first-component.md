...
3. Рот и две пары ног, каждая из которых может находится в одном из двух положений.
...
Изначально они отображаться не будут из-за того, что у их свойства «**visibility**» установлено значение «**hidden**».
...
В результате этого в рабочей области игры отобразиться SVG-образ динозавра с анимацией движения его ног.

![Окно с динозавром](learn\images\my-first-component\dino-window.png "Окно с динозавром")

Однако на данном этапе игра не будет реагировать ни на какие действия пользователя. Для ее начала необходимо зарегистрировать слушателя события нажатия клавиши клавиатуры «**keyup**». Это можно сделать в хуке компонента игры «**ready**» с помощью предопределенного метода «**listen**» следующим образом:
...
Здесь в качестве обработчика события регистрируется метод «**startGame**», который первоначально должен быть добавлен к компоненту «**oda-game**».
...
Это функция будет применяться к компоненту динозавра лишь в том случае, когда у него будет задан css-класс с именем «**dino-jump**». Добавить этот класс к этому компоненту можно с помощью метода «**jump**», который задается следующим образом:
...
Для отображения кактуса в области игры нужно первоначально подключить его модуль к файлу игры «**oda-game.js**» следующим образом:
...
При использовании этого тэга в индексном файле «index.html» кактус появится и начнет двигаться справа налево до полного выхода за левую границу рабочей области игры.

![Движущийся кактус](learn\images\my-first-component\cactus-move-window.png "Движущийся кактус")

Однако после окончания анимации кактус появится рядом с ней опять.
...
Первое из этих крыльев поднято вверх,
...
Для отображения тэга «**oda-pterodactyl**» не забудьте подключить модуль птеродактиля к файлу игры.
...
Эти компоненты будет создаваться циклически через опредленный временной интервал. Для этого заданы специальные счетчики их создания «**nextCloud**», «**nextCactus**» и «**nextPterodactyl**». Эти счетчика инициализируются случайным значением и уменьшаются на единицу с каждым следующим тактом анимации. Как только какой-либо счетчик уменьшиться до нуля, будет создаваться новый следующий компонент.
...
Счетчики появления облаков и кактусов изначально устанавливаются в ноль, а счетчик появления птеродактиля задан равным 50. В результате этого первое облако и кактус появятся сразу после начала игра, а первый птеродактиль будет создан после этого с небольшим временным интервалом.
...
В ней к методу «**checkDino**» обязательно необходимо привязать контекст компонента игры с помощью метода «**bind**». В противном случае указатель «**this**» внутри этого метода будет иметь неопределенное значение «**undefined**» и обратиться к элементам игры в теле метода «**checkDino**» будет невозможно.
...
В результате этого кактусы, облака и птеродактили начнут появляться в игре автоматически, когда пользователь запустит ее первый нажатием на клавишу пробела.
...
Кроме создания этих элементов, в методе «**checkDino**» можно предусмотреть определение момента столкновения динозавра с кактусом.
...
В этом методе находятся координаты текущего кактуса и динозавра с помощью стандартного метода «**getBoundingClientRect**», а затем определяется фак пересечения полигонов тела динозавра и его ног с полигоном кактуса.
...
Она в своем теле используют вспомогательный класс точек «**Point**», который тоже нужно задать в файле «**utils.js**» следующим образом:
...
Это функция для нахождения пересечения двух полигонов использует функцию «**intersectionLinePolygon**», которая позволяет найти пересечение одного полигона только с одной из линий другого полигона.
...
Если столкновение динозавра ни с каким кактусом не будет найдено, то эта же проверка продолжиться на следующем кадре анимации, когда кактусы изменят свою позицию.
...
В нем, кроме приостановки движения самого птеродактиля, приостанавливается и анимация взмахов его крыльев.

7. Удаляется слушатель нажатия на клавишу пробела для предотвращение последующих прыжков динозавра, разбившегося об кактус.
...
Он будет срабатывать при отпускании любой кнопки и вызывать метод «**continueGame**», который для этой цели нужно добавить к компоненту игры.
...
```
В нем кроме анимации движения птеродактиля, возобновляется и анимация взмахов его крыльев с помощью метода «**unpauseAnimations**».
...
9. Регистрируется опять обработчик «**dinoJump**» для возможности выполнения прыжков динозавром при нажатии на клавишу пробела.
...
Во время игры можно предусмотреть возможность прослушивания пользователем музыкальной композиции группы T-Rex «**Get It On**».

Для этого первоначально нужно создать папку «**audio**» и поместить в нее музыкальный файл этой группы с именем «**t-rex-get-it-on.mp3**».
...
В момент столкновения динозавра с кактусом ее проигрывание можно приостановить.  Для этого в методе «**stopMove**» компонента «**oda-dino**» достаточно у объекта «**audio**» вызвать метод «**pause**».
...
Чтобы продолжить проигрывать эту композиция с возобновлением игры, в методе «**continueMove**» динозавра у объекта «**audio**» нужно опять вызвать метод «**play**».
