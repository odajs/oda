...
Для этого можно добавить в папку игры «**js**» файл «**utils.js**» следующего содержания:
...
Эта функция в своем теле используют вспомогательный класс точек «**Point**».
...
Фактически функция «**pathToPolygon**» преобразует последовательность относительных точек, в последовательность абсолютных точек каждого переданного ей элемента SVG-элемента «path».

Для нахождения пересечения двух полигонов используется функция «**intersectPolygonPolygon**», которая также задана в js-модуле «**utils**» следующим образом:
...
Именно эта функции в итоге и определяет пересекается ли SVG-примитив динозавра с кактусом или нет.

Для использования необходимых функций из этого модуля «**utils**» их нужно подключить к модулю компонента динозавра «**oda-dino**».



Если пересечение будет найдено, то будет вызван метод «**gameOver**» и проверка пересечения с другими кактусами на этом этапе будет прервана.
В противном случае эта проверка продолжится на следующем кадре анимации.

Если пересечение все же произошло, то в методе окончания игры «**gameOver**» будут выполнены следующие действия:
...
2. Таймер счетчика набранных очков будет остановлен с использованием ранее записанного его уникального идентификатора в свойстве компонента **timerID**.

```javascript
  clearInterval(this.timerID);
```

3. Будет остановлена анимация динозавра вызовом метода «**stopMove**», который необходимо добавить к компоненту «**oda-dino**» следующим образом:
...
Здесь заданием стиля «**animationPlayState**» со значением  «**paused**» останавливается анимация прыжка динозавра, а с с помощью метода **pauseAnimations** приостанавливается анимация движения его ног. Кроме этого, у динозавра скрывается маленький глаз  и вместо него отображается большой глаз и рот. Этот эффект достигается изменением значения их атрибута «**visibility**».

4. Движение облаков будет приостановлено их методом «**stopMove**».
...
Метод «**stopMove**» нужно задать в компоненте «**oda-cloud**» следующим образом:
...
5. Будет остановлено движение всех кактусов.
...
Для этого в компоненте «**oda-cactus**» нужно задать метод «**stopMove**» таким же, как и у облаков.
...
6. Будет остановлено движение всех птеродактилей.
...
Для этого в их компоненте нужно задать метод «**stopMove**» следующий образом:
...
В этом методе кроме приостановки движения птеродактиля останавливается и анимация взмахов его крыльев.

7. Удаляется слушатель нажатия на клавишу пробела для осуществления прыжка динозавра.

```javascript
 this.unlisten('keydown', 'dinoJump', {target: document});
```

В результате выполнения метода «**Gave over**» все элементы игры будут остановлены.

![Gave over](learn\images\my-first-component\my-first-component1.png "Gave over")
