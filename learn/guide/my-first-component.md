...
Для быстрого доступа к SVG-части динозавра предусмотрим свойство **svg** в разделе **props**. Значение этого свойства зададим в хуке **attached**, который будет вызываться перед непосредственным добавлением компонента в DOM документа.

2. Компонент кактуса («oda-cactus») содержит только его SVG-приметив и стиль его отображения.
...
3. Компонент облака («oda-cloud») реализован аналогично компоненту кактуса. Он содержит SVG-приметив и стиль его отображения.
...
4. Компонент птеродактиля («oda-pterodactyl») кроме SVG-приметива его тела содержит крылья с эффектом анимации.
...
1. Файл  **style.css** со стилями отображения элементов игры.
...
Здесь переменная **--base-color** задает базовый цвет динозавра, облаков, птеродактиля и горизонта. Переменная **dark-color** определяет цвет основной области игры и глаз динозавра, а переменная **header-color** задает цвет текста заголовков. Фон игры и кактуса задан обычным серым цветом.
...
3. Стиль внешней обертки задает фор и размеры всей области игры.
...
5. Стиль заголовка игры шрифта и цвет надписей, а также их расположение.
...
6. Стиль надписи с количеством набранных очков задает отличный от стандартного представления надписей размер и верхний отступ.
...
7. Стиль надписи начала или окончания игры определяет смещение надписи относительно основной области игры.
...
8. Стиль отображения тиранозавра задает его начальное расположение.
...
Для начала игры зададим обработчик нажатия клавиш в основном файле с кодом игры **oda-dino-game** и привяжем его к событию нажатия клавиш клавиатуры на уровне документа.
...
В этом обработчике при нажатие на клавишу пробела (Space) проверяется закончилась ли на данный момент игра или она еще не начиналась. В этом случае выполняется метод **startGame**, который запускает игру на выполнение.

В этом методе создаются все элементы игры.
