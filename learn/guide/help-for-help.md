Для создания документации используется облегчённый язык разметки **Markdown**, который позволяет форматировать простой текст с сохранением максимальной его читаемости человеком.

Вся документация хранится в отдельных файлах с расширением **md**. Структура расположения этих файлов и вложенных папок на диске описывается в файле **_.dir**, который имеет след
