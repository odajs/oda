...
Внутри каждая такой папки должен находится служебны файл с именем «**_.dir**». В нем описывается структура всей папки в  следующем формате:
...
1. «**name**» — ключ, который определяет имя папки, в которой должны находиться все файлы, необходимые для описания работы данного компонента.

1. «**label**» — ключ, значение которого будет использоваться для вывода названия пункта меню на сайте для данного компонента.
...
Тогда в этой папке должны находится покрайней мере два элемента:

1. Папка с файлами самого компонента. Например, «**calculator**».
1. Служебный файл «**_.dir**» с описанием структуры этой папки.
...
1. Из папки «**tools**» в формировании оглавления на сайте сейчас будет использоваться только одна подпапка с именем «**calculator**».
1. Подпункт «**calculator**» в разделе «**tools**» («**Инструменты**») на сайте должен отображаться как «**Калькулятор**».
...
1. В оглавлении у пункта «**Калькулятор**» должно появится краткое описание, указанное в ключе **abstract**.

(https://odajs.org/learn/images/components/component5(2).png "Отображение подпункта в оглавлении")

1. При нажатии на этот пункт должен отображаться пример работы компонента, находящийся внутри папки **calculator**» и имеющий имя «**index.html**».


В эту папку помещают все элементы, необходимые для работы этого компонента и его документирования.
...
1. Файл с примером, демонстрирующим работу этого компонента. Например,  «**index.html**».
