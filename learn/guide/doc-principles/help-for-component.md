...
В папке каждого компонента должен находится файл с кодом данного компонента, написанном на языке JavaScript.

Существует негласное соглашения, что имя папки должно совпадать с именем файла файла компонента без учета расширения «**.js**».

Например, файла с кодом компонента «**calculator**» должен находится в папке «**calculator.js**» и иметь имя «**calculator.js**».

![JavaScript файл компонента] (https://odajs.org/learn/images/components/component2.png "JavaScript файл компонента")

При этом имя самого компонента должно совпадать с именем его файла с добавлением приставки «**oda**».

Это приставка указывается через символ дефиса «**-**», что является обязательным требованием технологии «**Web-components**» для формирования имен всех пользовательских компонентов.
