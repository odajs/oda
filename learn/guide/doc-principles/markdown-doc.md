...
Эти заголовки будут отображаться следующим образом:
...
```info
Заголовки более высокого уровня задать невозможно
```
...
Достаточно одно знака равно (=) или дефиса (-) на следующей строке без разделения пустой строкой. Остальные знаки указываются для красоты.

Заголовки других уровней так задать нельзя.
...
В конце документа обязательно должна быть указана одна пустая строка.

Текст переносится на следующую строку автоматически.

Для принудительного переноса текста на новую строку в ее конце необходимо поставить два пробела и нажать клавишу «Enter». Другим вариантом является использования тега <br/>.

Принудительно закончить строку можно поставив в еконце строки два пробела или добавить <br/>
