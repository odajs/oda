...
Корневой папкой документации является папка «**learn**», которая соответствует разделу «**Обучение**» главного меню сайта. Эта же папка соответствует вершине дерева оглавления в навигаторе на левой панели.

![Оглавление раздела](learn/images/doc-principles_table-of-contents-01.png"Пример оглавления раздела")

Для организации подразделов в корневой папке необходимо создать вложенные папки, а содержащиеся в них файлы в формате **Markdown** будут являться статьями в этих подразделах. Таким образом, структура документации определяется файловой структурой папки «**learn**».

Для включения подразделов и статей в документацию в папках должен находиться служебный файл с именем **\_.dir**. В нем задаются названия подразделов и статей, выводимые в оглавлении, и связь этих названий с соответствующими папками и файлами.

В файле **\_.dir** описывается только один уровень оглавления, определяемый содержимым текущей папки. Построения полной структуры оглавления осуществляется по содержимому файлов **\_.dir** из всех вложенных папок.

Оглавление в файле **\_.dir** описывается в формате JSON в виде одномерного массива из однотипных объектов. Каждый объект описывает отдельный пункт оглавления.

Объект, описывающий пункт оглавления, имеет формат:
...
1. Создать файл с учебным материалом в папке «**examples**», например с именем «**lesson.md**».
...
Более глубокие разделы отображаются только в оглавлении, в главное меню сайта они не попадают.
