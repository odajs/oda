...
Если не нужно отобрать иконку, то в заголовок оповещения необходимо добавить специальный модификатор «**hideicon**».
...
В этом случае левая граница рамки оповещения будет отображаться только определенным цветом, без иконки.
...
```info md
Обратите внимание, что между первым и вторым модификатором в качестве разделителя можно использовать обычный символом пробела. Все последующие модификаторы должны отделяться от предыдущих символом подчеркивания (**_**). Это особенность предусмотрена специально для большей наглядности выделения в заголовке типа оповещения.
```

Если иконка не отображается, то изменение типа оповещения будет приводить только к изменению цвета подсветки левой границы рамки оповещения
