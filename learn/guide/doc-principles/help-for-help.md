...
1. Модификатор Web-компонента «**[имя-компонента.js]**» говорит о том, что в тело HTML-документа, который используется для вывода результата, будет автоматически добавлен тэг пользовательского компонента с именем, указанным в квадратных скобках без расширения «**.js**».
    ...
    Этот тэг фактически станет хостом пользовательского компонента. Кроме этого, к заголовку документа вывода будет подключена библиотека фреймворка «oda.js». В результате этого, ODA-компонент, записанный в редакторе кода, будет появляться в окне вывода автоматически.
    ...
    Без этого модификатора тэг для пользовательского компонента пришлось бы создавать в ручном режиме.
    ...
    ```info hideGutter
    Следует отметить, что библиотека «oda.js» в любом случае подключется к документу вывода автоматически. Отдельно подключать ее в ручном режиме не нужно.
    ```

    Кроме этого, скрипт написанные в этом редакторе автоматически получает имя, указанное в квадратных скобках. Например, «**my-component.js**». Это имя можно будет использовать в дальнейшем для создания наследников данного компонента в других примерах.
