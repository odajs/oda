...
Теги ``` выделения кода в системе документация были расширены для создания механизма автоматического выполнения текста, записанного внутри них, в  режиме online.

Дополнительные теги указываются после начального тега кода через пробел.
...
Каждый дополнительный тег должен отделяться от другого знаком подчеркивания.
...
1. run — говорит, что код должен исполняться.
...
При использовании этого тега результат выполнения  будет выводит в окне, которое появится под самим кодом на сайте с документацией.

```javascript run
    newElement = document.createElement(<div>);
    newElement.innerHTML = `<h1>Привет {b}!</h1>`;
