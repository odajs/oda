...
```info md
Обратите внимание, что между первым и вторым модификатором в качестве разделителя можно использовать обычный символ пробела. Все последующие модификаторы должны отделяться от предыдущих только символом подчеркивания (**_**). Такая особенность предусмотрена специально для выделения первого модификатора в заголовке блока кода с целью его большей наглядности.
```

Кроме этого, порядок следования модификаторов значения не имеет. Их можно указывать в заголовке в любом порядке.
