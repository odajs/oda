...
1. Модификатор Web-компонента «**[имя-компонента.js]**» говорит о том, что при выводе результата в тело HTML-документа текущего блока будет автоматически добавлен тег пользовательского компонента с тем именем, которое будет указано в квадратных скобках без расширения «**.js**». Кроме этого, в заголовок этого документа будет автоматически подключена библиотека фреймворка «oda.js».
    ...
    В результате этого ODA-компонент будет отображаться в окне вывода результата автоматически.
    ...
    Если не указывать этот модификатор, то создавать тэг для пользовательского компонента пришлось бы в ручном режиме.
