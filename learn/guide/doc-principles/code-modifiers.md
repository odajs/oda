...
1. Модификатор Web-компонента «**[имя-компонента.js]**» говорит о том, что в тело HTML-документа, который используется для вывода результата, будет автоматически добавлен тэг пользовательского компонента с именем, указанным в квадратных скобках без расширения «**.js**».
    ...
    ```info hideGutter
    Следует отметить, что библиотека «oda.js» подключается к HTML-документу вывода автоматически только при указании модификатора «**javascript**» или при использовании модификатора Web-компонента «**[имя-компонента.js]**». Отдельно подключать ее в этих двух случаях не нужно.
    ```
