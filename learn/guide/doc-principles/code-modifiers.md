...
В системе документации синтаксис стандартного блок подсветки кода был расширен дополнительными модификаторами, которые позволяют увеличить функциональные возможности редактора кода. К таким модификаторам относятся:
...
1. Модификатор «**edit**» говорит о том, что текст в редакторе кода можно будет редактировать.
    ...
    Например, в следующим примере можно изменить любую строку кода, например, поменять тип элемента «**input**» и посмотреть к каким изменениям это будет приводить.
    ...
    ```info hideGutter
    Обратите внимание, что если во время редактирования код будет содержать ошибка, то в окне вывода никакой результат его выполнения выводится не будет.
    ```
...
1. Модификатор «**copy**» позволяет задать дополнительную кнопку в верхнем правом углу редактора, с помощью которой можно скопировать весь текст сообщения или кода.
    ...
    ```info hideGutter
    Обратите внимание, что если в заголовке не указать модификатор «**edit**», то выделить текст для копирования в редакторе будет нельзя. Поэтому кнопка «**copy**» будет единственной возможностью сделать это.
    ```
