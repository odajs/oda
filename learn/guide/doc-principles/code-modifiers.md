...
1. Модификатор «**edit**» говорит о том, что текст блока можно будет редактировать.
    ...
    Если в этом примере тип элемента  «**input**» изменить на «**radio**» или любой другой, то можно убедиться, что сделанные изменения немедленно отобразятся в области вывода.
...
1. Модификатор «**copy**» позволяет отобразить дополнительную кнопку в верхнем правом углу редактора, с помощью которой можно будет скопировать весь текст, написанный в блоке, без дополнительного его выделения.
    ...
    В этом случае нет необходимости выделять весь текст и нажимать сочетание клавиш «Ctrl+C», чтобы скопировать его. Это будет осуществляться автоматически при нажатии на кнопку «**copy**».
...
1. Модификатор родительского компонента «**{имя-родительского-компонента.js}**» позволяет к заголовку HTML-документа при выводе результата автоматически подключить скрипт с кодом родительского компонента.

    Для этого сначала необходимо объявить родительский компонент.
    ...
    Этот компонент будет выводится обычным образом, а его скрипт получит имя «**base-component.js**», которое указанно в квадратных скобках.
    ...
    Используя это имя, можно в блоке наследника подключить скрипт родителя, указав его имя в фигурных скобках.
    ...
    В этом случае в блоке наследника не придется повторно объявлять компонент родителя, усложняя код текущего примера.
    ...
    Если у компонента будет несколько родителей, то их имена нужно будет перечислить в фигурных скобках последовательно друг за другом, отделя одно имя от другого символом подчеркивания.
