...
1. Модификатор «**run**» позволяет не только подсвечивать код, но и выполнять его.
    ...
    Область вывода представляет собой плавающий фрейм (HTML-элемент «**iframe**»), в котором автоматически формируется стандартный HTML-документ. В заголовок этого документа автоматически добавляется тэг «**script**» с типом «**module**». В тело этого скрипта записывается код, указанный в редакторе. Этот документ и отображается в окне вывода результата. Поэтому любые элементы, добавленные в него, будут видны в области вывода.
...
1. Модификатор «**edit**» говорит о том, что текст в редакторе кода можно будет редактировать.
    ...
    Если одновременно указать оба модификатора «**run**» и «**edit**», то любое изменение кода будет мгновенно приводить к изменению результата его выполнения.

    Например, в следующим примере можно изменить любую строку кода, например, поменять тип элемента «**input**» с «**text**» на «**radio**» или на любой другой разрешенный тип.
    ...
    В результате этого все сделанные изменения в редакторе кода будут отображаться в области вывода в режиме реального времени, т.е. практически мгновенно.
