...
1. Модификатор Web-компонента «**[имя-компонента.js]**» говорит о том, что в тело HTML-документа, который используется для вывода результата, будет автоматически добавлен тэг пользовательского компонента с именем, указанным в квадратных скобках без расширения «**.js**».
    ...
    ```info hideGutter
    Следует отметить, что библиотека «oda.js» подключается к HTML-документу области вывода автоматически только при указании модификатора «**javascript**» или при использовании модификатора web-компонента «**[имя-компонента.js]**». Во всех остальных случаях ее придется подключать в ручном режиме.
    ```

    Например, при использовании модификатора типа «**html**» все необходимые элементы для вывода ODA-компонента в редакторе кода придется записывать самостоятельно.
