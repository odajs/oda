...
Для этого добавим к обычной игре «**Динозавр T-Rex**» js-модуль «**neural-net.js**», в котором определим класс «**NeuralNetwork**» для представления нейронной сети следующим образом:
...
В данном классе задается топология сети, архитектура которой передается конструктору в виде параметра с именем «**topology**».

```javascript
constructor (topology, clone)
```

Этот параметр по свой структуре является массивом, размер которого будет определять количество слоев нейросети. Значение каждого элемента этого массива будет задавать количество нейронов, которое необходимо создать на каждом этом слоев.

Значение этого параметра нужно сохранить в специально предусмотренном свойстве «**theTopology**» для дальнейшего использования в методах данного класса.

```javascript
this.theTopology = topology;
```

Второй параметр с именем «**clone**» предусматривает возможность досрочного выхода из конструктора без инициализации созданных слоев. Это позволяет немного оптимизировать процесс клонировании нейросети, когда веса будет задаваться определенными значениями, а не случайно.
...
Метод «**feedForward**» позволяет найти значение на выходе сети, используя переданное ему входное значение в списке параметров. Для этого он последовательное передает входные значения на каждый слой, получая их выходные значения, которые он передает в качестве входных значений следующему слою до тех пор, пока не будет получен окончательный результат на выходе самого последнего слоя.
...
Этот результат и возвращается в качестве выходного сигнала.
