...
Продемонстрируем это свойство на примере игры «**Динозавр T-Rex**», научив компьютер автоматически принимать решение о необходимости преодоления динозавром препятствий в виде кактусов.

Для этого к обычной игре «**Динозавр T-Rex**» добавим js-модуль «**neural-net**», в котором определим класс нейросети «**NeuralNetwork**» следующим образом:
...
В этом классе архитектуру сети будем задавать с помощью параметра «**topology**», который должен быть передан в начале конструктору.

```javascript
constructor (topology, clone)
```

По свой структуре этот параметр должен быть массивом. Эго размер будет определять количество слоев нейросети, а значение его элементов будет задавать количество нейронов на каждом этом слое.

Эту архитектуру нужно предварительно сохранить. Для этого нужно в классе «**NeuralNetwork**» задать дополнительное свойство «**theTopology**» следующим образом:

```javascript
this.theTopology = topology;
```

Второй параметр конструктора «**clone**» предусматривает возможность создания нейросети без инициализации ее слоев. Это позволяет немного оптимизировать процесс клонировании лучшей нейросети, когда веса нейронов должны задаваться не случайно, как в конструкторе, а на основе весов нейронов другой нейросети.

```javascript
if (clone) return;
```

Если же нейросеть создается заново, то в конструкторе будет созданы ее слои в виде объектов специального класса «**NeuralSection**».
...
Описание это класса будет дано чуть позже.

Метод «**clone**» класса «**NeuralNetwork**» позволяет создать клон текущей нейросети. Для этого в нем создается новая сеть с помощью предыдущего конструктора, но веса ее нейронов на каждом слое задаются такими же, как и веса нейронов текущей сети.
...
```
Именно здесь используется второй параметр конструктора «**clone**» со значением «**true**» для оптимизации процесса клонирования.
...
Для этого входное значение, переданное ему, поступает на вход первого слоя нейросети. На его выходе получается значение, которое должно быть передано на вход следующего слоя. Этот метод так последовательно и передает выходные значения предыдущего слоя на вход следующего слоя до тех пор, пока не будет получено окончательное значение на выходе самого последнего слоя. Это значение и будет возвращаться как выходное значение всей нейросети.
...
Каждый слой нейронов представляет собой отдельный объект специального класса «**NeuralSection**». Этот класс нужно объявить в том же самом модуле, что и класс «**NeuralNetwork**», следующим образом:
