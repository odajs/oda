...
Это позволяет немного оптимизировать процесс клонировании, когда веса нейронов новой сети должны быть заданы на основе нейронов другой сети.

Если параметр «**clone**» будет иметь значение «**false**», то нейросеть будет создана со случайными весами. Для этого используется специальный класс «**NeuralSection**», структура которого будет рассмотрена позже.
...
Здесь для каждого слоя создается отдельный объект с типом «**NeuralSection**», у которого указывается количество нейронов, которое должно быть на его входе и выходе.

Метод «**clone**» класса «**NeuralNetwork**» позволяет создать копию текущей нейросети. Для этого в нем создается новая сеть с той же самой архитектурой и с такими же самыми весами нейронов, что и у текущей сети. Именно здесь для оптимизации процесса создания клонируемой сети конструктору передается параметр «**clone**» со значением «**true**».
