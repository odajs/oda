...
Метод «**clone**» класса «**NeuralNetwork**» позволяет создать копию текущей нейросети. Для этого в нем создается новая сеть с той же самой архитектурой и с такими же весами нейронов, что и у текущей сети.
...
Именно здесь для оптимизации процесса копирования конструктору передается параметр «**clone**» со значением «**true**». Сам же процесс клонирования осуществляется вызовом метода «**clone**» объекта класса «**NeuralSection**».
