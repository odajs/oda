...
Для использования это значения в дальнейшем его нужно сохранить в специально предусмотренном для этого свойстве «**theTopology**» класса «**NeuralNetwork**».
...
Это позволяет немного оптимизировать процесс клонировании нейросети, когда веса ее нейронов должны быть заданы на основе другой уже существующей сети.

Если параметр «**clone**» будет иметь значение «**false**», то все нейроны новой сети будут созданы со случайными весами с помощью специально предусмотренного для этого класса «**NeuralSection**».
...
Здесь каждый слой нейросети создается в виде отдельного объекта с типом «**NeuralSection**» с указанием количества нейронов, которое должно быть на его его входе и на его выходе.

Метод «**clone**» класса «**NeuralNetwork**» позволяет создать копию текущей нейросети. Для этого в нем создается новая сеть, но с той же самой архитектурой и с такими же весами нейронов, что и у текущей сети. Именно здесь конструктору передается параметр «**clone**» со значением «**true**». В результате этого нейроны изначально случайными значениям не инициализируются.
...
Сам процесс клонирования осуществляется вызовом метода «**clone**», специально предусмотренного для этого в классе слоев нейросети «**NeuralSection**» .
...
Для этого ему передается входное значение в виде параметра «**input**». С его помощью формируется сигнал на выходе первого слоя, который затем передается на вход следующего слоя, и так далее, пока не будет получено значение на выходе самого последнего слоя. Это значение и будет выходным значением всей нейросети.
...
1. «**inputCount**» – количество нейронов на входе.
1. «**outputCount**» – количество нейронов на выходе.

Для каждого входного нейрона создается отдельный массив весов, которые будет связывать его со всеми выходными нейронами.

```javascript
this.weights = new Array(inputCount + 1);
```

Размер этого массива задается на единицу больше количества входных нейронов этого слоя из-за необходимости представления фиктивного нейрона смещения. На его вход всегда будет подаваться единичный входной сигнал, что позволит избежать потери реакции нейросети на изменение весов нейронов при нулевом уровне сигналов на всех остальных ее входах. Кроме этого, нейрон смещения позволяет изменять  целевую функции на выходе, быстрее находя правильное решение.
...
Она фактически имеет линейную зависимость, уменьшающая влияние отрицательные выходных сигналов в двадцать раз.
...
Для обучения этой нейросети будем использовать игру «**Динозавр T-Rex**», которая была создана в уроке «**Мой первый компонент**».
