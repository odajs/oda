...
Продемонстрируем это свойство на примере игры «**Динозавр T-Rex**», научив компьютер автоматически принимать решение о необходимости преодоления динозавром препятствия в виде кактуса.

Для этого добавим к обычной игре «**Динозавр T-Rex**» js-модуль «**neural-net.js**», в котором объявим класс «**NeuralNetwork**» для представления нейронной сети.
...
В данном классе задается топология сети. Ее архитектура передается конструктору в виде параметра с именем «**topology**».

```javascript
constructor (topology, clone)
```

Этот параметр является массивом, длина которого определяет количество слоев нейросети. Значение каждого элемента этого массива задает количество нейронов, которое будет находится на соответствующем слое.

Значение этого параметра сохраняется в специальном свойстве «**theTopology**» данного класса и будет использоваться в дальнейшем в его методах.

```javascript
this.theTopology = topology;
```

Второй параметр с именем «**clone**» предусматривает возможность досрочного выхода из конструктора без инициализации созданных слоев, немного оптимизируя его работу при клонировании нейросети.
