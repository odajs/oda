...
В этом классе архитектура нейросети задается с помощью параметра «**topology**», который передается конструктору класса в списке параметров:

```javascript
constructor (topology, clone)
```

По свой структуре этот параметр является массивом, размер которого будет определять количество слоев нейросети, а значение его элементов будет задавать количество нейронов на соответствующих слоях.

Для дальнейшего использования этого параметра его значение предварительно сохраняется в свойстве «**theTopology**» класса нейросети.

```javascript
this.theTopology = topology;
```

Второй параметр конструктора «**clone**» предусматривает возможность создания нейросети без инициализации ее слоев. Это позволяет немного оптимизировать процесс клонировании лучшей нейросети, когда веса нейронов должны задаваться не случайно, как в конструкторе, а на основе другой нейросети.
