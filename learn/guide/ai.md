...
Это позволяет немного оптимизировать процесс клонировании, когда веса нейронов новой сети будут задаваться на основе весов нейронов другой нейросети.

Если параметр «**clone**» будет имеет значение «**false**», то нейросеть будет создана со случайными весами. Для этого используется специальный класс «**NeuralSection**», структура которого будет рассмотрена позже.
...
Здесь для каждого слоя создается отдельный объект с типом «**NeuralSection**» с заданным количество входных и выходных нейронов.
