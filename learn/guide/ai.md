...
Для этого добавим к обычной игре «**Динозавр T-Rex**» js-модуль «**neural-net.js**», в котором определим класс «**NeuralNetwork**».
...
Этот класс будет представлять мозг динозавра в виде нейронной сети. Архитектура это сети задается с помощью параметра «**topology**», который передается конструктору класса следующим образом:

```javascript
constructor (topology, clone)
```

По свой структуре этот параметр является массивом, размер которого будет определять количество слоев нейросети, а значение каждого его элемента будет задавать количество нейронов на соответствующем слое.

Для дальнейшего использования этого параметра сохраним его в свойстве «**theTopology**» нашего класса.

```javascript
this.theTopology = topology;
```

Второй параметр конструктора с именем «**clone**» предусматривает возможность создания сети без инициализации ее слоев. Это позволяет немного оптимизировать процесс клонировании, когда веса нейронов будут задаваться не случайным образом, а на основе другой нейросети.
