...
Если с помощью метода **$next** необходимо вызвать не функцию, а метод какого-то компонента, то к нему обязательно будет нужно привязать контекст этого компонента с помощью метода **bind**.
...
При вызове анонимной функции, контекст которой зависит от способа её вызова, указатель **this** в строгом режиме будет не определен, т.е. будет иметь значение **undefined**.
...
Из-за этого при вызове первой функции возникнет ошибка обращения к объекту **undefined**. Во второй функции контексте будет привязан к текущему компоненту в ручном режиме с помощью метода **bind** и никакой ошибки возникать не будет.

Если при вызове метода **$next** указать задержку в ноль тактов, то с его помощью методы или функции будут вызываться сразу же на следующем такте анимации без какой-либо задержки.
...
Если второй параметр у метода **$next** явно не указать, то он будет иметь нулевой значение по умолчанию.
...
Данный пример будет работать аналогично предыдущему примеру. Оба метода в нем будут выполняться сразу друг за другом на следующем такте анимации.
