...
В этом примере значение свойства **myStyle** передается через директиву **~style** атрибуту **style** элемента **div**, в следствие чего у него изменяется стиль отображения.
...
Значение директивы **~style** можно задать с помощью свойства компонента, но и строковыми литералами.
...
При использовании шаблонных литералов обратные одинарные кавычки (**`**), называемые грависами, придется экранировать символом обратного слэша **\\**, так как такие кавычки уже используются при записи свойства **template** самого компонента, а использовать одинаковые вложенные кавычки в строках запрещено.
...
Вместо строковых литералов в качестве значения директивы **~style** можно использовать обычный объект. В этом случае каждое его свойство будет соответствовать отдельному CSS-объявлению.
...
Однако если использовать верблюжью нотацию (Camel case), то неразрешенное имя свойства будет автоматически преобразовано в правильную форму. Использовать апострофы в этом случае уже будет не нужно.
