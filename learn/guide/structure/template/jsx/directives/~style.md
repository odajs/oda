...
В этом примере значение свойства **myStyle** с помощью директивы **~style** будет присвоено атрибуту **style** HTML-элемента **div**, в следствие чего он изменить свой стиль отображения.

В отличие от обычного присвоения стиля использование директивы **~style** позволяет включить механизм реактивности, так как при этом любое изменение свойства компонента будет автоматически приводить к изменению стиля его отображения. Например:
...
Если для указания строковых шаблонных литералов использовать обратные одинарные кавычки (**`**), называемые грависами, то их придется экранировать символом обратного слэша **\**, так как такие кавычки уже используются для задания свойства **template** самого компонента и указывать другие такие же кавычки внутри них без экранирования запрещено.
...
В этом случае значения CSS-свойств придется указывать в кавычках, так как они должны иметь строковый тип.

Если CSS-свойство имеет неразрешенное с точки зрения языка JavaScript имя, то в объекте это имя придется тоже записывать в одинарных кавычках.
...
Такое преобразование автоматические происходит не только при использовании строковых литералов, но и при задание CSS-свойств с помощью отдельного объекта.
...
Следует отметить, что механизм реактивности работает только при изменении ссылки на сам объект с CSS-стилями.
...
Изменении значений отдельных свойств этого объекта к появлению реактивности отображения стиля не приводит.
...
Для включения этого механизма в директиве **~style** достаточно указать объект с использованием литеральной нотации и связать необходимые его свойства со значением свойств самого компонента.
...
В этом случае любое изменение соответствующего свойства компонента, будут автоматические приводить к изменению связанного с ним CSS-объявления.

Если значение директивы **~style** указано в строковой нотации, то включить механизм реактивности можно  будет только с использованием шаблонных литералы с интерполяционными подстановками, которые записывается с помощью знака доллара и фигурных скобок.
