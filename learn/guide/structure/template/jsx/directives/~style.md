...
В этом примере значение свойства компонента **myStyle** передается через директиву **~style** атрибуту **style** элемента **div**, в следствие чего у него изменяется стиль его отображения.

В отличие от обычного присвоения директива **~style** позволяет использовать механизм реактивности, т.е. автоматически изменять стиль отображения соответствующего HTML-элемента с изменением значения связанного с ним свойства компонента.
...
При использовании шаблонных литералов придется экранировать символ обратной одинарной кавычки (**`**), так как такие кавычки уже используются при записи значения свойства **template** компонента, а использовать вложенные кавычки одного и того же типа без экранирования в языке JavaScript запрещено.
...
В качестве значения директивы **~style** вместо строковых литералов можно использовать обычный объект. В этом случае каждое свойство этого объекта будет соответствовать отдельному CSS-объявлению стиля HTML-элемента.
...
Если CSS-объявление имеет неразрешенное с точки зрения языка JavaScript имя, то это имя тоже необходимо будет записать в кавычках.
...
Однако если использовать верблюжью нотацию (Camel case), то неразрешенное имя будет автоматически преобразовано в правильную форму, с добавлением дефиса (**-**) между словами. Использовать апострофы в этом случае уже будет не нужно.
...
Автоматическое преобразование имен из верблюжьей нотацию в шашлычную (Kebab case) происходит и для любых свойств объекта, если он указывается в директиве **~style**.
...
Для его включения сам объект необходимо записать в литеральной форме, а в качестве значения его свойств требуется указать имя свойства компонента, с которым это значение должно быть связанно.
...
``` warning_md
В этом случае символ **$** обязательно должен быть заэкранирован, иначе интерполяционное выражение будет применятся к свойству **template** компонента, а не к значению директивы **~style**.
```
...
Заметьте, если шаблонный литерал записан внутри обычной строки, то механизм реактивности работать не будет.
...
В этом примере шаблонный литерал рассматривается лишь как часть строки и директивой **~style** он как интерполяционное выражение не воспринимается.

```warning_md
У самого компонента нельзя задавать свойство с именем **style**, так как такое свойство уже задано в классе **HTMLElement**, наследниками которого являются все ODA-компоненты. Если же такое свойство будет задано по ошибке, то оно перекроет унаследованное родительское свойство **style** со всеми его CSS-объявлениями.
```
