...
При использовании шаблонных литералов придется экранировать символ обратной одинарной кавычки (**`**), так как такие кавычки уже используются при записи значения свойства **template** компонента, а использование вложенных кавычек одного и того же типа без экранирования в языке JavaScript запрещено.
...
В качестве значения директивы **~style** вместо строковых литералов можно использовать обычный объект. В этом случае каждое свойство этого объекта будет соответствовать отдельному CSS-объявлению стиля отображения HTML-элемента.
...
Если в CSS-объявлении используется неразрешенное с точки зрения языка JavaScript имя, то это имя тоже должно быть записано в кавычках.
...
Однако если использовать верблюжью нотацию (Camel case), то неразрешенное имя будет автоматически преобразовано в правильную форму, с добавлением дефиса (**-**) между словами. Использовать апострофы в этом случае уже нет необходимости.
...
Автоматическое преобразование имен из верблюжьей нотацию в шашлычную (Kebab case) происходит и для любых свойств объекта, если он указан в директиве **~style**.
...
Для его включения сам объект необходимо записать в литеральной форме, а в качестве значения его свойств нужно будет указать имя свойства компонента, с которым это значение должно быть связанно.
...
В этом примере шаблонный литерал рассматривается лишь как часть строки и директивой **~style** как интерполяционное выражение отдельно обрабатываться не будет.
