...
В отличие от обычного присвоения директива **~style** позволяет использовать механизм реактивности, т.е. автоматически изменяет стиль отображения соответствующего HTML-элемента с изменением значения связанного с ним свойства компонента.
...
Автоматическое преобразование имен из верблюжьей нотацию в шашлычную (Kebab case) происходит и для любых свойств объекта при использовании его в директиве **~style**.
...
Для его включения сам объект необходимо записать в литеральной форме, а в качестве значений его свойств нужно будет указать имя того свойства компонента, с которым это значение должно быть связанно.
...
```warning_md
Не задавайте у самого компонента свойство с именем **style**, так как такое свойство уже задано в классе **HTMLElement**, наследниками которого являются все компоненты. Если Вы это сделаете, то это приведет к возникновению фатальной ошибки.
```
