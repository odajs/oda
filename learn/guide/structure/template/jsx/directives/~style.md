...
При использовании шаблонных литералов придется экранировать обратные одинарные кавычки (**`**), так как такие кавычки уже используются при записи значения свойства **template** компонента, а использовать вложенные кавычки одного и того же типа без экранирования в языке JavaScript запрещено.
...
Вместо строковых литералов в качестве значения директивы **~style** можно использовать обычный объект. В этом случае каждое его свойство будет соответствовать отдельному CSS-объявлению стиля отображения элемента.
...
Обратите внимание, что в отличие от строковых литералов значения свойств у такого объекта необходимо указывать в кавычках, так как они должны обязательно иметь строковый тип. Например, **'green'**.

Если имя CSS-свойства имеет неразрешенное с точки зрения языка JavaScript значение, то это имя тоже придется записать в кавычках.
...
Однако если использовать верблюжью нотацию (Camel case), то неразрешенное имя будет автоматически преобразовано в правильную форму, с добавлением дефиса **-** между словами. Использовать апострофы в этом случае уже не нужно.
...
Автоматическое преобразование имен из верблюжьей нотацию в шашлычную (Kebab case) происходит и для любых свойств объекта, указанного в директиве **~style**.
...
Следует отметить, что механизм реактивности работает только при изменении всей ссылки на связанный со стилем объект.
...
Для его включения сам объект необходимо записать в литеральной форме, а в качестве значения его свойств указать имя соответствующего свойства компонента, с которым это значение должно быть связать.
